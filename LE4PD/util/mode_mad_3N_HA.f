...
c	nfrs=10000 !just for testiing!
	...
	write(*,*)"delr",delr
	hnorm=0.0
	open(unit=21,file='com_HCINVmatrix',status='old')
	do i=1,3*n
	  do j=1,3*n
	    read(21,*)qinvm(i,j)
	  end do
...
c	if(a.eq.4)write(*,*)itheta,iphi,hisang(a,itheta,iphi)
	...
	DO a=1,3*n
	  WRITE(aa,*)a
	  aa=adjustl(aa)
	  OPEN(unit=25,file='anly_HA_'//TRIM(aa)//'.dat',status='unknown')
...
c	delphi=(2.*pi)/(real(iphi))
	...
	write(cbins,*)nbins
	cbins=adjustl(cbins)
	open(unit=110,file="fricorr_mp_"//trim(cbins)//"_3N_HA.dat")
	open(unit=111,file="feminmax_mp_"//trim(cbins)//"_3N_HA.dat")
	open(unit=112,file="femin_mp_"//trim(cbins)//"_3N_HA.dat")
	do a=1,3*n
	do i=1,nbins/2-1
	iphi=nint(nbins*sin(i*delr))
c	delphi=(2.*pi)/(real(iphi))
...
! typical barrier from ground from median absolute deviation
	open(unit=111,file="fmad_mp_"//trim(cbins)//"_3N_HA.dat")
	do a=1,3*n
	  imad=0
	  fmad=0.0
	  fmadord=0.0
...
!	open histogram files
	...
	write(ii,*)i
	ii=adjustl(ii)
	open(unit=100+i,file='fempa_'//trim(ii)//'_3N_HA.dat')
...
c	write(200+a,*)
	...
	!DO a=1,10 !Only the first ten for now
	!  WRITE(aa,*)a
	!  aa=adjustl(aa)
	!  OPEN(unit=24,file='xi_'//TRIM(aa)//'_aniso_HA.dat')
	!  DO k=1,nfrs
