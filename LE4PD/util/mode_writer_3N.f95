	...
	open(unit=10,file="com_Rmatrix",status='old')
	open(unit=11,file="com_RTmatrix",status='old')
	do i=1,3*n
	  do j=1,3*n
	    READ(10,*)Q(i,j)
	    READ(11,*)QINV(i,j)
	...
	DO a=1,3*n
	  ...
	  OPEN(unit=25,file='xi_'//TRIM(aa)//'.xvg',status='unknown')
	  OPEN(unit=27,file='xi_'//TRIM(aa)//'_aniso.xvg',status='unknown')
	  ...
	  DO k=1,nfrs
	    ...
	    WRITE(25,*)xi(a,k)
            WRITE(26,*)xix(a,k),xiy(a,k),xiz(a,k)
            mean = mean + xi(a,k)
            var = var + xi(a,k)**2
	  END DO
	  CLOSE(25)
